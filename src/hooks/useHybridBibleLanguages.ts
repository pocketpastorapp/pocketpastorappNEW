...
export const useHybridBibleLanguages = () => {
  ...
  useEffect(() => {
    const loadLanguages = async () => {
      try {
        ...
        // Auto-select only if no language is already chosen
        setSelectedLanguage(prev => {
          if (prev) return prev; // keep externally chosen value
          ...
          if (englishLanguage) return englishLanguage.id;
          return languageData.length > 0 ? languageData[0].id : '';
        });
      } catch (error) {
